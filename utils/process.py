...
def adj_to_bias(adj, sizes, nhood=1):
    ...
    for g in range(nb_graphs):
        ...
        for _ in range(nhood):
            ...
            mt[g] = np.matmul(adj[g], adj[g])*0.1 + adj[g] + np.eye(adj.shape[1])   #邻接矩阵+单位阵 （加了个自循环）
...
def sparse_to_tuple(sparse_mx):
    ...
    def to_tuple(mx):
        if not sp.isspmatrix_coo(mx):
            mx = mx.tocoo()     #将mx转换成coo格式的稀疏矩阵
        coords = np.vstack((mx.row, mx.col)).transpose()    #二维array，每一行代表一个非零元素的坐标
        values = mx.data    #非零值
        shape = mx.shape
...
def preprocess_adj_2_bias(adj):
    num_nodes = adj.shape[0]
    adj_2 = 0.01*(adj * adj) + adj + sp.eye(num_nodes)
    adj_2[adj_2 > 1.0] = 1.0
    #adj_2[adj_2 < 1.0] = -1e9
